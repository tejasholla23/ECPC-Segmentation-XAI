...
def export_report_to_file(patient_data: dict, file_format: str = "markdown") -> str:
    """
    Export the clinical report to a file and return the path.
    """
    if not patient_data or not patient_data.get("patient_id"):
        return None
    ...
    safe_id = "".join([c if c.isalnum() else "_" for c in patient_id])
    ...
    dice_label, _ = interpret_dice_score(dice)
    conf_label, _ = interpret_confidence(conf)
    volume_desc = interpret_lesion_volume(volume)

    if file_format == "markdown":
        filename = f"reports/Report_{safe_id}.md"
        content = f"""# AI Clinical Analysis Report
**Patient ID:** {patient_id}
**Report Timestamp:** {timestamp}
**Analysis System:** ECPC-AI Pipeline v1.0

## 1. Segmentation Assessment
- **Dice Score:** {dice:.3f}
- **Confidence:** {conf:.1%}
- **Assessment:** {dice_label} ({conf_label})

## 2. Lesion Characteristics
- **Estimated Volume:** {volume:.1f} mm³
- **Clinical Observation:** {volume_desc}

## 3. Radiomics Summary
- **Mean Density:** {float(metrics.get('mean_density', 0.0)):.1f} HU
- **Sphericity:** {float(metrics.get('sphericity', 0.0)):.2f}
- **Max Diameter:** {float(metrics.get('max_diameter', 0.0)):.1f} mm

## 4. AI Recommendation
{ "Action Required: Immediate radiologist audit recommended." if dice < 0.60 else "Proceed with standard clinical workflow." }

---
*Generated by ECPC-Segmentation-XAI Dashboard*
"""
    else:
        # Placeholder for HTML/PDF
        filename = f"reports/Report_{safe_id}.html"
        content = generate_clinical_report_html(patient_data)
