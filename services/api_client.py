...
class APIClient:
    ...
    def normalize_response_data(response: Dict) -> Dict:
        ...
        normalized = {
            ...
            "metrics": {
                "dice_score": float(response.get("metrics", {}).get("dice_score", 0.0)),
                "iou_score": float(response.get("metrics", {}).get("iou_score", 0.0)),
                "hausdorff_distance": float(response.get("metrics", {}).get("hausdorff_distance", 0.0)),
                "sensitivity": float(response.get("metrics", {}).get("sensitivity", 0.0)),
                "specificity": float(response.get("metrics", {}).get("specificity", 0.0)),
            },
...
def _generate_dummy_patient_data(patient_id: str) -> Dict:
    ...
    return {
        ...
        "metrics": {
            "dice_score": np.random.uniform(0.75, 0.95),
            "iou_score": np.random.uniform(0.65, 0.90),
            "hausdorff_distance": np.random.uniform(5.0, 15.0),
            "sensitivity": np.random.uniform(0.75, 0.95),
            "specificity": np.random.uniform(0.80, 0.98),
        },
        "status": "completed",
